...
def test_first_call_fetches_full_range_and_caches():
    e = make_engine()
    src = FakeSource()
    bars = get_price_history("AAPL", date(2026, 1, 1), date(2026, 1, 31),
                             source=src, engine=e)
    assert src.calls == [(date(2026, 1, 1), date(2026, 1, 31))]
    assert len(bars) == 2
    # second identical call hits cache only -> no new source call
    bars2 = get_price_history("AAPL", date(2026, 1, 1), date(2026, 1, 31),
                              source=src, engine=e)
...
def test_widening_range_fetches_only_edges():
    e = make_engine()
    src = FakeSource()
    get_price_history("AAPL", date(2026, 1, 10), date(2026, 1, 20), source=src, engine=e)
    src.calls.clear()
